...
if(_in_box){	//入盒
	if(box_idx!=3){					//进错盒，则报错并返回原位置
	...
	else if(_array[0]=="0"){	//盒中存在空位
		show_debug_message("Yellow In box!");
		...
		_array[0] = "yellow_bucket";
		origin_box_id = _new_box_id;
	}else{		//盒中没有空位，需要返回初始位置
		show_debug_message("Yellow No free space!");
		x = _initial_x;
		y = _initial_y;
	...
	show_debug_message("Yellow Out of box!");
	x = _pos_x;
	y = _pos_y;
	//_array[0] = "0";
	if(origin_box_id!=0){		//之前在某个盒子中
