...
if(worker==1) _controller.worker_1[0]="0";
if(worker==2) _controller.worker_2[0]="0";
if(worker==3) _controller.worker_3[0]="0";
if(worker==0) global.in_box_cnt+=1;
	
if(place_meeting(x,y,obj_worker1)){
	if(_controller.worker_1[0]=="0" || _controller.worker_1[0]=="red"){
		x = _controller.hat_x_pos[0];
		y = _controller.hat_y_pos[0];
		worker = 1;
		_controller.worker_1[0]="red";
	}else{
		worker = 0;
		x = origin_x;
		y = origin_y;
	...
	if(_controller.worker_2[0]=="0" || _controller.worker_2[0]=="red"){
		x = _controller.hat_x_pos[1];
		y = _controller.hat_y_pos[1];
		worker = 2;
		_controller.worker_2[0]="red";
	}else{
		x = origin_x;
		y = origin_y;
		worker = 0;
	...
	if(_controller.worker_3[0]=="0" || _controller.worker_3[0]=="red"){
		x = _controller.hat_x_pos[2];
		y = _controller.hat_y_pos[2];
		worker = 3;
		_controller.worker_3[0]="red";
	}else{
		x = origin_x;
		y = origin_y;
		worker = 0;
...
for(var _i=0;_i<3;_i+=1){
	if(_woker_arr[_i]!="red" && _woker_arr[_i]!="0"){
		_right_worker = false;
	}
}
