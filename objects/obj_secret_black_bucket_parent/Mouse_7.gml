...
else if(origin_box_id==0) _origin_array = global.array_black;
...
if(_in_box){	//入盒
	...
	if(box_idx==3 || box_idx==4){					//进错盒，则报错并返回原位置
		...
		pos_idx = _i;
		_array[pos_idx] = "black";
		origin_box_id = _new_box_id;
		show_debug_message("black bucket x={0}, y={1}", x,y);
	}
...
if(_back_origin){				//回到初始框
	...
	show_debug_message("Black bucket Out of box!");
	x = _pos_x+_i*96;
	y = _pos_y;
...
show_debug_message(global.array_black);
origin_x = x;
origin_y = y;
