...
if(_in_box){	//入盒
	...
	if(_i<global.arr_len){	//盒中存在空位
		show_debug_message("Black drop 4 In box!");
		x = _pos_x+_i*80;
		y = _pos_y;
		...
		pos_idx = _i;
		_array[pos_idx] = "black_drop4";
		origin_box_id = _new_box_id;
	}else{		//盒中没有空位，需要返回初始位置
		show_debug_message("Black drop 4 No free space!");
		x = _initial_x;
		y = _initial_y;
	...
	show_debug_message("Black drop 4 Out of box!");
	x = _pos_x;
	y = _pos_y;
	//_array[0] = "0";
	if(origin_box_id!=0){		//之前在某个盒子中
