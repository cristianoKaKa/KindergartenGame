...
if(place_meeting(x,y,obj_worker1)){
	if(_controller.worker_1[1]=="0" || _controller.worker_1[1]=="red"){
		x = _controller.jacket_x_pos[0];
		y = _controller.jacket_y_pos[0];
		worker = 1;
		_controller.worker_1[1]="red";
	}else{
		worker = 0;
		x = origin_x;
		y = origin_y;
	...
	if(_controller.worker_2[1]=="0" || _controller.worker_2[1]=="red"){
		x = _controller.jacket_x_pos[1];
		y = _controller.jacket_y_pos[1];
		worker = 2;
		_controller.worker_2[1]="red";
	}else{
		x = origin_x;
		y = origin_y;
		worker = 0;
	...
	if(_controller.worker_3[1]=="0" || _controller.worker_3[1]=="red"){
		x = _controller.jacket_x_pos[2];
		y = _controller.jacket_y_pos[2];
		worker = 3;
		_controller.worker_3[1]="red";
	}else{
		x = origin_x;
		y = origin_y;
		worker = 0;
