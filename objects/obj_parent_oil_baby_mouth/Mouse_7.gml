...
if(place_meeting(x, y, obj_oil_baby_1)){
	if(global.mouth_array[0]!="0"){	//箱内已满，无法进箱
		x = last_x;
		y = last_y;
	...
	if(in_box){			//之前在箱中，先出箱再进箱
		global.mouth_array[last_box_id] = "0";
	}else{				//之前不在箱中，box_cnt加一			
		global.in_box_cnt += 1;
	}
	in_box = true;
	_box_id = 0;
	x = global.pos_mouth[_box_id][0];
	y = global.pos_mouth[_box_id][1];
	global.mouth_array[_box_id] = "1";
}else if(place_meeting(x, y, obj_oil_baby_2)){
	if(global.mouth_array[1]!="0"){	//箱内已满，无法进箱
		x = last_x;
		y = last_y;
		audio_play_sound(snd_beep_error,0,false);
		return;
	...
	if(global.mouth_array[2]!="0"){	//箱内已满，无法进箱
		x = last_x;
		y = last_y;
		audio_play_sound(snd_beep_error,0,false);
		return;
	...
	if(global.mouth_array[3]!="0"){	//箱内已满，无法进箱
		x = last_x;
		y = last_y;
		audio_play_sound(snd_beep_error,0,false);
		return;
